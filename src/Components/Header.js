...
function Header(props) {
  ...
  return (
    <div className='flex  justify-between items-center h-[70px] mx-1  md:mx-4'>
      ...
      <div className='text-[13px] md:text-[16px] text-gray-500'>Welcome  <span className='text-[13px] md:text-[16px]  text-gray-900'>{user.bio.firstname}</span> </div>
      </div>

        <div className='flex gap-3 items-center'>
            <div style={{zIndex:0}} className={`border-2 rounded-xl z-10 z-20 z-40  flex items-center justify-center md:h-[50px] absolute md:static bg-white h-[70px] w-full left-0  top-0 px-2 py-2 gap-1 ${!openSearch && 'hidden'}  md:flex`}>
            ...
            <div className='relative ' style={{zIndex:-1}} >
              {
                ...
                <BsBell size={22} style={{zIndex:0}} className='text-gray-500'/>
            </div>
